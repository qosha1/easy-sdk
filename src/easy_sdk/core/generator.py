...
class DjangoDocsGenerator:
    ...
    def __init__(
        ...
        # Track generated files across all generators
        self.generated_files: List[Path] = []
    ...
    def generate_all(self) -> GenerationResult:
        ...
        try:
            with Progress(console=self.console) as progress:
                ...
                self.generated_files.extend(typescript_result.generated_files)
                progress.update(main_task, advance=15)
    ...
    def generate_sphinx_docs(self, analysis_result: Optional[Dict] = None) -> GenerationResult:
        """
        ...
        try:
            ...
            sphinx_files = self.sphinx_generator.generate_documentation(analysis_result)
    ...
    def _compile_statistics(self) -> Dict[str, int]:
        ...
        # Get actual counts from scan results
        total_apps = 0
        total_serializers = 0
        total_views = 0
        processed_apps = 0
        
        if hasattr(self.scanner, 'scan_result') and self.scanner.scan_result:
            scan_result = self.scanner.scan_result
            total_apps = len(scan_result.discovered_apps)
            
            # Count processed apps
            for app_name in scan_result.discovered_apps.keys():
                if self.config.should_include_app(app_name):
                    processed_apps += 1
        
        # Count analyzed serializers and views from analysis results  
        if self.analysis_result:
            for app_name, app_analysis in self.analysis_result.items():
                if isinstance(app_analysis, dict):
                    serializers = app_analysis.get('serializers', [])
                    views = app_analysis.get('views', [])
                    total_serializers += len(serializers) if isinstance(serializers, list) else 0
                    total_views += len(views) if isinstance(views, list) else 0
        
        stats = {
            'Total Apps': total_apps,
            'Processed Apps': processed_apps,
            'Generated Files': len(self.generated_files),
            'Serializers Analyzed': total_serializers,
            'Views Analyzed': total_views,
            'TypeScript Interfaces': total_serializers,  # One interface per serializer
        }
